...
export class DraftingNotaDinasPage {
    ...
    goToKonsepNaskahNotaDinas() {
        ...
        selectedKonsep.should('contain', 'Surat Biasa')
            .then((val) => {
                const selectJenisNaskah = cy.xpath(nota_dinas.selectJenisNaskah).as('selectJenisNaskah')
                selectJenisNaskah.select('Nota Dinas').should('have.value', '/konsep-naskah/nota-dinas')
            })
    }
